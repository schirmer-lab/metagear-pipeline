...
Quality control for shotgun-metagenomic DNA reads. Produces decontaminated, adapter-trimmed paired reads that are ready for any of the analysis workflows.

## What it does

1. **FastQC** (v0.12.1) on the raw reads — pre-QC report.
...
4. **KneadData** (v0.10.0, internally running Trimmomatic 0.39) for host decontamination. Process arg: `--trimmomatic-options 'HEADCROP:15 SLIDINGWINDOW:4:15 MINLEN:50' --reorder --remove-intermediate-output --bypass-trf`. Reads are aligned against the host genome(s) supplied via `--kneaddata_refdb` and host-matching pairs are dropped.
