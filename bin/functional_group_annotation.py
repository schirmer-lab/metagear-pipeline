...
def _load_FG_interproscan_Pfam(interproscan_fp,ann_tool,start_pos_idx=6,pfam_id_idx=4,eval_idx=8,prot_len_idx=2):
    ann_dic = {}
    ...
    with open(interproscan_fp,"r") as f:
        for line in f:
            ...
            if protein_id not in ann_dic:
                ann_dic[protein_id] = []
            ann_dic[protein_id].append([prot_len,start_pos,end_pos,pfam_id,pfam_ann,cur_eval])
    return ann_dic

def _calculate_functional_group(interproscan_fp,FG_sfp,ann_tool="Pfam",start_pos_idx=6,pfam_id_idx=4,eval_idx=8,prot_len_idx=2):

    ann_dic = _load_FG_interproscan_Pfam(interproscan_fp,ann_tool,start_pos_idx,pfam_id_idx,eval_idx,prot_len_idx)
    # return: {protein_id:[length,start,end,Pfam_id,Pfam_description,eval]}
    ...
    for cur_protein_id in ann_dic:
        ...
        cur_start_lst__sorted_idx = sorted(range(len(cur_start_lst)), key=lambda i: cur_start_lst[i])
