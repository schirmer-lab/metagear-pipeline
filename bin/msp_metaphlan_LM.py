...
def _load_metaphlan_profile(metaphlan_profile_fp,sep=","):
	"""
	Load MetaPhlAn relative abundance profile.

	Parameters:
		metaphlan_profile_fp (str): Path to MetaPhlAn profile file.
		sep (str): Delimiter used in the file.
	...
	with open(metaphlan_profile_fp,"r") as f:
		for ii,line in enumerate(f):
			if ii==0:
				spp_lst = line.strip().split(sep)[1:]
				pos2spp = dict(zip(list(range(len(spp_lst))),spp_lst))
				for spp in spp_lst:
					ret_abd_prev[spp] = []
				continue
			lst = line.strip().split(sep)
			ret_sample_lst.append(lst[0].replace("_paired","")) 
			for ii,abd in enumerate(lst[1:]):
				ret_abd_prev[pos2spp[ii]].append(float(abd))
	# reformat the abundance list by sample_id
	ret_abd = {}
...
def _linear_fit_Apair(cur_msp_id,cur_spp,metaphlan_abd,msp_abd,sample_id_lst):
	...
	"""
	cur_metaphlan_abd = np.array([float(metaphlan_abd[cur_spp][cur_sample_id]) for cur_sample_id in sample_id_lst])
	cur_metaphlan_abd = cur_metaphlan_abd.reshape((len(cur_metaphlan_abd), 1))
	cur_msp_abd = np.array([float(msp_abd[cur_msp_id][cur_sample_id]) for cur_sample_id in sample_id_lst])
...
def linear_fit_all(full_res_sfp,msp_id_lst,spp_lst,metaphlan_abd,msp_abd):
	...
	for cur_msp_id in msp_id_lst:
		for cur_spp in spp_lst:
			res[(cur_spp,cur_msp_id)] =  _linear_fit_Apair(cur_msp_id,cur_spp,metaphlan_abd,msp_abd,sample_id_lst)
	
	with open(full_res_sfp,"w") as sf:
		sf.write("\t".join(["metaphlan_spp","msp_id","coefficients","interceptions","MSE","r2"])+"\n")
		for cur_spp,cur_msp_id in res:
			sf.write("\t".join([cur_spp,cur_msp_id]+[str(float(val)) for val in res[(cur_spp,cur_msp_id)]])+"\n")
	return
	...
	with open(fp,"r") as f:
		for ii,line in enumerate(f):
			...
			if (len(sel_msp_id_lst)>0) and (msp_id not in sel_msp_id_lst):
				continue
			if (len(sel_metaphlan_spp_lst)>0) and (metaphlan_spp not in sel_metaphlan_spp_lst):
				continue
			ret[(metaphlan_spp,msp_id)] = [float(it) for it in [coefficients,interceptions,MSE,r2]]
	return ret


def _msp_taxaANN_metaphlanLM(output_dir,msp_profile_fp,metaphlan_profile_fp,metaphlan_version = "v3"):
	# load abd tables from both msp and metaphlan
	msp_abd = _load_msp_profile(msp_profile_fp)
	metaphlan_abd = _load_metaphlan_profile(metaphlan_profile_fp)

	msp_id_lst = list(msp_abd.keys())
	spp_lst = list(metaphlan_abd.keys())
	...
	# save everything
	linear_fit_all(full_res_sfp,msp_id_lst,spp_lst,metaphlan_abd,msp_abd) # only run once for each cohort!
	...
	with open(bestR2_res_sfp,"w") as sf:
		sf.write("\t".join(["msp_id","metaphlan_spp","coefficients","interceptions","MSE","r2"])+"\n")
		cur_msp_id_lst = list(res_best_R2.keys())
		cur_msp_id_lst.sort()
		for cur_msp_id in cur_msp_id_lst:
			cur_spp = res_best_R2[cur_msp_id][0]
			sf.write("\t".join([cur_msp_id,cur_spp]+[str(float(val)) for val in res_best_R2[cur_msp_id][1:] ])+"\n")
	return # done		
...
# metaphlan_profile_fp = "/nfs/arxiv/shen/CLD_KCH_2025/analysis/metaphlan3/analysis/s__known_abundance.csv"
